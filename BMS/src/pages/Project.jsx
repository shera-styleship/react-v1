...
const Project = () => {
	...
	const [companyOptions, setCompanyOptions] = useState([{ value: 'all', label: '전체 회사', companyNo: null }]);
	...
	const fetchCompanies = async () => {
		...
		try {
			const res = await axios.get('https://bmsapi.styleship.com/api/Common/companies', {
				headers: {
	...
	useEffect(() => {
		...
		const found = projects.find((p) => {
			const id = p.workNo ?? p.projectNo ?? p.id ?? p.workListNo ?? p.workID ?? null;
	...
	// worklists post 받기 전 임시 기능 구현
	const updateWorkTeamLocal = (workNo, nextWorkTeam) => {
		// 리스트 업데이트
		setProjects((prev) =>
			(prev || []).map((p) => (String(p.workNo) === String(workNo) ? { ...p, workTeam: nextWorkTeam } : p))
		);

		// 상세(선택된 프로젝트)도 업데이트
		setSelectedProject((prev) =>
			prev && String(prev.workNo) === String(workNo) ? { ...prev, workTeam: nextWorkTeam } : prev
		);
	};
	// worklists post 받기 전 임시 기능 구현
			...
			<div className='project-wrap'>
				<div>
					{loading ? (
						...
						<ProjectList projects={projects} onSelect={handleSelect} companyOptions={companyOptions} />
					)}
				</div>
				<div>
					<ProjectView project={selectedProject} onSaveWorkTeam={updateWorkTeamLocal} />
				</div>
			</div>
		</div>
	);
